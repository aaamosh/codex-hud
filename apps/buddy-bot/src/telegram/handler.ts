...
import { callMatchmaker } from "../matchmaker";
...
export async function handleTelegramUpdate(update: TelegramUpdate, env: Env, repo: D1BuddyRepository): Promise<void> {
  const telegram = new TelegramClient(env.TELEGRAM_BOT_TOKEN);
  const sourceUser = update.message?.from ?? update.callback_query?.from;
  if (!sourceUser) return;

  const now = new Date().toISOString();
  const user = await repo.upsertTelegramUser(toUserInput(sourceUser), now);
  const service = new BuddyService(repo, secretsFromEnv(env));
  ...
  if (update.callback_query) {
    ...
    if (!parsed) {
      await telegram.sendMessage(sourceUser.id, "I did not understand that button. Try /status.");
      return;
    }

    if (parsed.kind === "match") {
      const outbox = await callMatchmaker(env, "/match-action", {
        matchId: parsed.matchId,
        action: parsed.action,
        actorUserId: user.id,
        actorChatId: user.telegram_user_id
      });
      await sendOutbox(telegram, outbox, async (message) => {
        if (message.afterSend?.clearSeekerEmailCiphertextId) {
          await repo.clearSeekerEmailCiphertext(message.afterSend.clearSeekerEmailCiphertextId);
        }
      });
      return;
    }

    await handleSimpleCallback(parsed.action, parsed.value, { env, repo, telegram, service, user });
    return;
  }
  ...
  if (text.startsWith("/")) {
    await handleCommand(text, { env, repo, telegram, service, user });
    return;
  }

  const conversation = await repo.getConversation(user.id);
  if (!conversation) {
    await telegram.sendMessage(message.chat.id, "Send /give or /seek to start.");
    return;
  }

  if (conversation.flow === "give") {
    await advanceGiveConversation(text, { env, repo, telegram, service, user });
    return;
  }
  if (conversation.flow === "seek") {
    await advanceSeekConversation(text, { env, repo, telegram, service, user });
  }
}
...
async function handleCommand(commandText: string, context: HandlerContext): Promise<void> {
  ...
  switch (command) {
    case "/start":
      await context.telegram.sendMessage(context.user.telegram_user_id, copy.start, { replyMarkup: startKeyboard() });
      return;
    case "/give":
    ...
    case "/cancel":
      ...
      return;
    case "/confirmed":
      await context.telegram.sendMessage(context.user.telegram_user_id, "Use the buttons inside the current match to confirm. If the buttons are gone, open /status.");
      return;
    case "/rules":
      await context.telegram.sendMessage(context.user.telegram_user_id, copy.rules);
      return;
    case "/help":
      await context.telegram.sendMessage(context.user.telegram_user_id, copy.help);
      return;
    case "/admin":
      await sendAdminHint(context);
      return;
    default:
      await context.telegram.sendMessage(context.user.telegram_user_id, "Unknown command. /help shows available actions.");
  }
}
...
async function handleSimpleCallback(action: string, value: string | undefined, context: HandlerContext): Promise<void> {
  switch (action) {
    ...
    case "give_plan":
      ...
      await context.telegram.sendMessage(context.user.telegram_user_id, "The form is shorter now. How many active invite slots do you want to offer?");
      return;
    case "seek_availability":
      await finishSeekConversation(Number(value ?? 60), context);
      return;
    default:
      await context.telegram.sendMessage(context.user.telegram_user_id, "That button is stale. Try /status.");
  }
}

async function startGive(context: HandlerContext): Promise<void> {
  await context.repo.setConversation(context.user.id, { flow: "give", step: "capacity", data: {} }, new Date().toISOString());
  await context.telegram.sendMessage(context.user.telegram_user_id, "How many active invite slots do you want to offer? For example: 1 or 3.");
}
...
async function startSeek(context: HandlerContext): Promise<void> {
  ...
  await context.telegram.sendMessage(context.user.telegram_user_id, copy.privacyConsent);
}
...
async function advanceGiveConversation(text: string, context: HandlerContext): Promise<void> {
  ...
  if (conversation.step === "capacity") {
    const result = await context.service.createGiverOffer({
      user: context.user,
    ...
    await context.repo.clearConversation(context.user.id);
    if (!result.ok) {
      await context.telegram.sendMessage(context.user.telegram_user_id, result.message);
      return;
    }
    await context.telegram.sendMessage(
      context.user.telegram_user_id,
    ...
    await sendOutbox(context.telegram, await callMatchmaker(context.env, "/attempt-pending", {}));
  }
}
...
async function advanceSeekConversation(text: string, context: HandlerContext): Promise<void> {
  ...
  if (["language", "region", "timezone"].includes(conversation.step)) {
    await context.repo.setConversation(context.user.id, { flow: "seek", step: "email", data }, new Date().toISOString());
    await context.telegram.sendMessage(context.user.telegram_user_id, "The form is shorter now: language and region are no longer needed.");
    await context.telegram.sendMessage(context.user.telegram_user_id, copy.privacyConsent);
    return;
  }
  if (conversation.step === "email") {
    if (!isValidEmail(text)) {
      await context.telegram.sendMessage(context.user.telegram_user_id, "That does not look like an email. Check the format and send it again.");
      return;
    }
    data.email = text;
    await context.repo.setConversation(context.user.id, { flow: "seek", step: "availability", data }, new Date().toISOString());
    await context.telegram.sendMessage(context.user.telegram_user_id, "Can you act quickly after a match?", { replyMarkup: availabilityKeyboard() });
    return;
  }
...
async function finishSeekConversation(availabilityWindowMinutes: number, context: HandlerContext): Promise<void> {
  ...
  if (!conversation || conversation.flow !== "seek") {
    await context.telegram.sendMessage(context.user.telegram_user_id, "Start a request with /seek.");
    return;
  }
  const data = conversation.data as Record<string, unknown>;
  if (!data.email) {
    await context.telegram.sendMessage(context.user.telegram_user_id, "The request is incomplete. Start again with /seek.");
    await context.repo.clearConversation(context.user.id);
    return;
  }
  const result = await context.service.createSeekerRequest({
    user: context.user,
    language: DEFAULT_MATCH_LANGUAGE,
    region: DEFAULT_MATCH_REGION,
    timezone: DEFAULT_MATCH_TIMEZONE,
  ...
  await context.telegram.sendMessage(context.user.telegram_user_id, "Request created. Looking for a match.");
  await sendOutbox(context.telegram, await callMatchmaker(context.env, "/attempt-match", { seekerRequestId: result.value.id }));
}
...
async function sendStatus(context: HandlerContext): Promise<void> {
  ...
  await context.telegram.sendMessage(context.user.telegram_user_id, body);
}

async function sendAdminHint(context: HandlerContext): Promise<void> {
  const allowed = (context.env.ADMIN_TELEGRAM_IDS ?? "")
    .split(",")
    .map((id) => id.trim())
    .filter(Boolean);
  if (!allowed.includes(context.user.telegram_user_id)) {
    await context.telegram.sendMessage(context.user.telegram_user_id, "Admin access is not configured for this Telegram ID.");
    return;
  }
  await context.telegram.sendMessage(context.user.telegram_user_id, "Admin surface: open the Worker `/admin` URL and use ADMIN_TOKEN as the Bearer token.");
}
...
export async function sendOutbox(
  ...
  for (const message of outbox) {
    try {
      ...
      // Do not log message text; a relay message can contain plaintext email.
    }
  }
}
