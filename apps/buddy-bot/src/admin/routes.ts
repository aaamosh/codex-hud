...
adminRoutes.use("*", async (c, next) => {
  if (!isAuthorized(c.req.raw, c.env.ADMIN_TOKEN)) {
...
adminRoutes.get("/", async (c) => {
  ...
  const config = configRows.length > 0 ? parseRuntimeConfig(Object.fromEntries(configRows.map((row) => [row.key, JSON.parse(row.value_json)]))) : DEFAULT_CONFIG;
  const token = new URL(c.req.url).searchParams.get("token") ?? "";
  return c.html(renderAdmin(snapshot, config, token));
});

adminRoutes.get("/api/snapshot", async (c) => {
  const repo = new D1BuddyRepository(c.env.DB);
  return c.json(await repo.getAdminSnapshot());
});
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    body {
      ...
      color: var(--text);
      font-family: Satoshi, Geist, IBM Plex Sans, Segoe UI Variable, sans-serif;
      letter-spacing: 0;
    }
  ...
  <script>
    ...
    document.addEventListener("click", async (event) => {
      ...
      if (button.dataset.action) {
        await api("/admin/api/matches/" + button.dataset.id + "/" + button.dataset.action, { method: "POST" });
      }
      if (button.dataset.block) {
        await api("/admin/api/users/" + button.dataset.user + "/" + button.dataset.block, { method: "POST" });
      }
      location.reload();
    });
  </script>
